...
bool load_json_config(const std::string& filepath, Slic3r::DynamicPrintConfig& config, bool verbose = false) {
    ...
    try {
        ...
        std::string reason;
        int ret = config.load_from_json(filepath, substitution_context, true, key_values, reason);
        if (ret != 0) {
            std::cerr << "Error parsing JSON config";
            if (!reason.empty())
                std::cerr << ": " << reason;
            std::cerr << "\n";
            return false;
...
void set_default_config(Slic3r::DynamicPrintConfig& config) {
    ...
    if (filament_nozzle_map_opt->values.empty()) {
        // Nozzle IDs in LayeredNozzleGroupResult are zero-based. Keeping the
        // standalone default aligned avoids a late auto-map rewrite during
        // Print::process() for simple single-nozzle slices.
        filament_nozzle_map_opt->values = {0};
    }
