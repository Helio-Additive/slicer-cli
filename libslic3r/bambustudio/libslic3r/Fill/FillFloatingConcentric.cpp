...
#include <libslic3r/Fill/FillFloatingConcentric.hpp>
    ...
    for (int i = 0; i < (int)lines.size(); ++i) {
        ...
        if (thickness_delta > tolerance) {
            ...
            if (thickness_delta > tolerance) {
                ...
                const bool is_a_floating = line.is_a_floating;
                const bool is_b_floating = line.is_b_floating;
                // delete this line and insert new ones
                lines.erase(lines.begin() + i);
                for (size_t j = 0; j < segments; ++j) {
                    FloatingThickline new_line(pp[j], pp[j + 1],width[2 * j],width[2 * j+1], is_a_floating,is_b_floating);
...
FloatingThickPolylines FillFloatingConcentric::resplit_order_loops(Point curr_point, std::vector<const Arachne::ExtrusionLine*> all_extrusions, const ExPolygons& floating_areas, const Polygons& sparse_polys, const coord_t default_width)
{
    ...
    for (size_t idx = 0; idx < all_extrusions.size(); ++idx) {
        ...
        if(print_object_config->detect_floating_vertical_shell.value)
        {
            ...
            // Grid contours keep pointers into the supplied point buffers.
            // Move the points into storage that outlives all grid queries.
            std::vector<Points> grid_polylines;
            grid_polylines.emplace_back(std::move(polyline.points));
            EdgeGrid::Grid grid;
            grid.set_bbox(bbox_line);
            grid.create(grid_polylines, scaled<coord_t>(10.), !all_extrusions[idx]->is_closed);
