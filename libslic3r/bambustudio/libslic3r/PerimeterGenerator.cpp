...
#include <cstdlib>
namespace {
    static FILE *pg_overhang_trace_file() {
        static FILE *f = nullptr;
        static bool tried = false;
        if (!tried) {
            tried = true;
            const char *path = std::getenv("OVERHANG_TRACE");
...
static void detect_bridge_wall(const PerimeterGenerator &perimeter_generator, ExtrusionPaths &paths, const Polylines &remain_polines, ExtrusionRole role, double mm3_per_mm, float width, float height)
{
    for (Polyline poly : remain_polines) {
        ...
        if (line.length() < poly.length()) {
            if (FILE *tf = pg_overhang_trace_file()) {
                std::fprintf(tf, "BRIDGE_WALL role=%d deg=%d kind=curved-overhang len=%.1f linelen=%.1f npts=%zu\n",
                             (int)role, overhang_sampling_number - 1, poly.length(), line.length(), poly.points.size());
            }
        ...
        // bridge wall
        if (FILE *tf = pg_overhang_trace_file()) {
            std::fprintf(tf, "BRIDGE_WALL role=%d deg=%d kind=bridge len=%.1f linelen=%.1f npts=%zu\n",
                         (int)role, overhang_sampling_number, poly.length(), line.length(), poly.points.size());
        }
        extrusion_paths_append(paths,
                               std::move(poly),
...
static ExtrusionEntityCollection traverse_loops(const PerimeterGenerator &perimeter_generator, const PerimeterGeneratorLoops &loops, ThickPolylines &thin_walls)
{
    ...
    for (const PerimeterGeneratorLoop &loop : loops) {
        ...
        if (perimeter_generator.config->detect_overhang_wall && perimeter_generator.layer_id > perimeter_generator.object_config->raft_layers) {
            ...
            if (FILE *tf = pg_overhang_trace_file()) {
                std::fprintf(tf, "LOOP layer=%d role=%d is_ext=%d ovh_speed=%d inside=%zu remain=%zu polylen=%.1f\n",
                             (int)perimeter_generator.layer_id, (int)role, (int)is_external,
                             (int)detect_overhang_speed, inside_polines.size(), remain_polines.size(),
                             to_polyline(polygon).length());
            }
