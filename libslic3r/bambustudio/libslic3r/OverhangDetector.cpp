...
#include <cstdlib>
namespace {
    static FILE *overhang_trace_file() {
                                ...
                                ExtrusionPaths   &paths)
    {
        ...
        for (size_t polyline_idx = 0; polyline_idx < middle_overhang_polyines.size(); ++polyline_idx) {
            ...
            for (SplitPoly &polylines_collection : out) {
                if (FILE *tf = overhang_trace_file()) {
                    std::fprintf(tf, "CLASSIC_DOD role=%d len=%.1f deg=%.6f npts=%zu\n",
                                 (int)role, polylines_collection.polyline.length(),
                                 polylines_collection.degree,
                                 polylines_collection.polyline.points.size());
                }
