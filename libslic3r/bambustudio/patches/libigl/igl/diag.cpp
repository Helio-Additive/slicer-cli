...
template <typename T> IGL_INLINE void igl::diag(const Eigen::SparseMatrix<T>& X, Eigen::SparseVector<T>& V) {
...
template <typename T> IGL_INLINE void igl::diag(const Eigen::SparseVector<T>& V, Eigen::SparseMatrix<T>& X) {
    // clear and resize output
    Eigen::SparseMatrix<T, Eigen::RowMajor> dyn_X(V.size(), V.size());
    dyn_X.reserve(Eigen::VectorXi::Constant(V.size(), 1));
    // loop over non-zeros
...
template void igl::diag<double, Eigen::Matrix<double, -1, -1, 0, -1, -1>>(
    Eigen::SparseMatrix<double, 0, int> const&, Eigen::MatrixBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>>&);
template void igl::diag<double>(Eigen::SparseMatrix<double, 0, int> const&, Eigen::SparseVector<double, 0, int>&);
template void igl::diag<double, Eigen::Matrix<double, -1, 1, 0, -1, 1>>(
    Eigen::MatrixBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> const&, Eigen::SparseMatrix<double, 0, int>&);
