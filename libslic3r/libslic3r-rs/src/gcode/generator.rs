...
impl GCodeStats {
    ...
    pub fn calculate_filament_stats(&mut self, config: &PrintConfig) {
        ...
        // BambuStudio labels the header field [cm^3] but actually outputs mm³,
        // so we keep mm³ here for parity.
        ...
        // Weight uses the *stats* density (defaults to 0.0), matching
        // BambuStudio which outputs weight=0.00 when density is unset.
        self.filament_weight_g = self.filament_volume_cm3 * self.filament_density;
    }
...
impl GCodeHeader {
    ...
    pub fn with_raw_settings(
        ...
        Self {
            software_version: "BambuStudio 02.05.01.52".to_string(),
            model_print_time,
    ...
    pub fn generate_config_block(&self) -> String {
        if let Some(ref settings) = self.raw_settings {
            if let Some(obj) = settings.as_object() {
                ...
                // Sort keys alphabetically to match BambuStudio output
                let mut keys: Vec<&String> = obj.keys().collect();
                keys.sort();
                // Keys that BambuStudio excludes from CONFIG_BLOCK
                let excluded_keys = [
