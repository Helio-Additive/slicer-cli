...
use crate::{
    arachne::{ArachneConfig, ExtrusionLine as ArachneExtrusionLine},
    clipper_utils::{difference, grow, offset2, opening, shrink, union_ex, OffsetJoinType},
    extrusion_entity::{
...
impl PerimeterGenerator {
    ...
    fn generate_classic_one(&self, slice: &ExPolygon) -> PerimeterResult {
        ...
        // union_ex is used to normalize the single ExPolygon (handles any self-intersections
        // introduced by simplification). We process ONE ExPolygon, not all slices together.
        let mut last = if surface_simplify_resolution > 0.0 {
            // TODO: Port simplify_p properly; for now just wrap in union_ex for normalization
            union_ex(&[slice.clone()])
        } else {
            vec![slice.clone()]
        };
        ...
        loop {
            ...
            // TODO: Port top_one_wall_type handling (lines 1099-1250)
