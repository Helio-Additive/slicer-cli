...
## Current parity (ROUND 99 — see memory `project_byteparity_admesh.md` + `project_nondeterminism_roots.md` for the full round log; this branch's copy jumps R81→R96 because `alex/frame-unify` forked at R81. R82–R95 are on `alex/libslic3r-parity-engine` @e35e290 + memory.)

> **Branch note:** the gated slice-byte-match stack (R83 simplify, R84 F1-union, R85–R87 frame-unify/Eigen shim, R88 two-pass union, R91 simplify_p, R96–R98 below) lives on `alex/frame-unify` @f3c3036, all behind env gates (`F1_UNION`/`FRAME_UNIFY`/`SLICE_SIMPLIFY`/`SLICE_CENTER`), default parity unchanged. R99 (determinism) is on the DEFAULT path. Nothing pushed.

- **★★ R99 — RUST GCODE NOW DETERMINISTIC (default path @f3c3036). BANK POINT for this arc.** Rust emitted different gcode every run (a hard gate on byte-parity); root class = the port mirrored C++ `std::unordered_map`/`unordered_set` (stable-per-run) with Rust `HashMap`/`HashSet` (per-run-random `RandomState`), + one ASLR-leaking pointer-set. 4 sites fixed (one dominant Arachne cascade): #1 `fill_rectilinear` ACO `thread_rng`→faithful `mt19937_64` seed 5489 (ISO-verified, native-matching); #2 `region_expansion` bridge-dir HashMap→BTreeMap; #3 `fill_floating_concentric` `.iter().next()`→`.min()`; #4 `skeletal_trapezoidation` wall-start ptr-set→min-by-geometric-key; +`tool_ordering` sweep. Result: rust 3× byte-identical, material-neutral (8/10 gcode features byte-identical; 2 fill-chaining features move within pre-fix jitter), slices 238/239 + R65 intact. Determinism holds on the gated path too. **MEASURED gated byte-parity gap (F1_UNION+FRAME_UNIFY+SLICE_SIMPLIFY+SLICE_CENTER, inline config): native 139498 vs rust 146958 lines (+5.3%).** Concentration: (1) DOMINANT — ISI/floating fill-CLASSIFICATION split (Internal-solid-infill 389 vs 242; Floating 122 vs 167) = the pre-existing R69–R77 `process_external`/geo-clipper-@1000 fragmentation; (2) ARC over-segmentation (G2/G3 11923 vs 14265, +19.6%; R79b); (3) walls/gap/sparse near-match in count but not byte-exact; first divergence is a trivial machine-start template diff (`G29.1` vs `M629`). Material at native (−0.1%). **DECISION: BANKED here** — finishing is a multi-session structural effort (classification F1-walk + arc-fitting + per-feature + seam); the biggest single lever is the ISI/floating classification (process_external @1000→@1e5, R71–R77 infra exists but the material split didn't close there).
