...
## Current parity (ROUND 100 — see memory `project_byteparity_admesh.md` + `project_nondeterminism_roots.md` for the full round log; this branch's copy jumps R81→R96 because `alex/frame-unify` forked at R81. R82–R95 are on `alex/libslic3r-parity-engine` @e35e290 + memory.)

> **Branch note:** the gated slice-byte-match stack (R83 simplify, R84 F1-union, R85–R87 frame-unify/Eigen shim, R88 two-pass union, R91 simplify_p, R96–R98, R100 below) lives on `alex/frame-unify` @658b92e, all behind env gates (`F1_UNION`/`FRAME_UNIFY`/`SLICE_SIMPLIFY`/`SLICE_CENTER`), default parity unchanged. R99 (determinism) is on the DEFAULT path. Branch PUSHED to origin (2026-07-02; standing policy: always push alex/* as backup, never main).
