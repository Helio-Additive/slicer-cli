                                  ...
                                  int32_t *out_indices, int64_t *out_n_tris)
{
    ...
    if (getenv("COLDBG")) {
        ...
        auto mix = [&h](uint32_t x) { h ^= x; h *= 1099511628211ULL; };
        for (int64_t i = 0; i < 3 * n_verts; ++i) { union {float f; uint32_t u;} c; c.f = verts[i]; mix(c.u); }
        printf("COLDBG-S entry nv=%lld nt=%lld vh=%llx\n", (long long)n_verts, (long long)n_tris, (unsigned long long)h);
        size_t negz=0; for (int64_t i = 0; i < n_verts; ++i) { union {float f; uint32_t u;} c; c.f=verts[3*i+2]; if (c.u==0x80000000u) negz++; }
        union {float f; uint32_t u;} a,b,cc; a.f=verts[0]; b.f=verts[1]; cc.f=verts[2];
        printf("COLDBG-S negz=%zu v0=%x,%x,%x\n", negz, a.u, b.u, cc.u);
        for (int64_t i = 1000; i < 1005; ++i) { union {float f; uint32_t u;} x,y,z; x.f=verts[3*i]; y.f=verts[3*i+1]; z.f=verts[3*i+2]; printf("COLDBG-S v%lld=%x,%x,%x\n", (long long)i, x.u, y.u, z.u); }
        uint64_t hd = 1469598103934665603ULL;
        auto mixd = [&hd](uint32_t x) { hd ^= x; hd *= 1099511628211ULL; };
        for (float f : min_vertex_dot_product) { union {float f_; uint32_t u;} c; c.f_ = f; mixd(c.u); }
        printf("COLDBG-S dots dh=%llx\n", (unsigned long long)hd);
    }
    ...
    while (face_indices.size() > (size_t) target_triangle_count) {
        ...
        if (getenv("COLDBG")) printf("COLDBG-S round before=%zu after=%zu\n", prev_size, face_indices.size());
    }
