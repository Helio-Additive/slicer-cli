...
/// flatten to `Polygons`. DIVERGENCE: ClipperLib uses EvenOdd fill internally for SimplifyPolygons,
/// but the crate's `union_polygons_ex` uses the geo backend (non-zero). See module notes.
...
impl WallToolPaths {
    ...
    pub fn separate_out_inner_contour(&mut self) {
        ...
        // DIVERGENCE: the crate clipper backend exposes only a non-zero union (union_polygons_ex).
        // We use that here; the even-odd fill rule for self-overlapping wall contours is not
        // available. See module notes.
    ...
    pub fn get_first_wall_contour(&mut self) -> &Polygons {
        ...
        if !self.toolpaths_generated && self.inset_count > 0 {
            ...
            // WallToolPaths.cpp:791-793  return EmptyPolygons;
            self.first_wall_contour.clear();
            return &self.first_wall_contour;
        }
