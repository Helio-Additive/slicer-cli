...
// `SurfaceDetectionConfig` + the surface-classification free fns are defined at the
// bottom of this module (relocated from surface.rs) and re-exported at module root.
    ...
    pub fn prepare_infill(&mut self) -> Result<()> {
        ...
        for region_id in 0..self.num_printing_regions() {
            ...
            if !use_faithful_vshell {
                ...
                discover_vertical_shells(&mut region_surfaces, &surface_cfg);
                if self.canceled.load(std::sync::atomic::Ordering::Relaxed) {
                    return Err(crate::Error::Cancelled);
                }
            }
            ...
            if crate::debug::topdbg::enabled() {
                for (idx_layer, layer_surfaces) in region_surfaces.iter().enumerate() {
                    ...
                    crate::debug::topdbg::dump_top_surfaces(idx_layer, "d2_vshell_top", layer_surfaces);
                }
            }
...
// ===========================================================================
// Surface-classification pipeline (relocated from surface.rs, 2026-06-14).
//
// These free functions are the divergent reimplementation of BambuStudio's
// PrintObject surface-detection (detect_surfaces_type / discover_vertical_shells /
// discover_horizontal_shells / process_external_surfaces) and LayerRegion::
// prepare_fill_surfaces. They live here (PrintObject's structural home) rather
// than in surface.rs, which mirrors only Surface.{cpp,hpp}. Re-exported at the
// print_object module root so existing `crate::print_object::*` call sites resolve.
// ===========================================================================
pub use surface_classification::*;
mod surface_classification {
    use crate::clipper_utils::{
...
pub fn split_internal_surfaces(
