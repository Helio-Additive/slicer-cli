...
pub fn merge_slices(
    ...
    match op {
        ...
        CSGType::Difference => {
            // target[i] = diff_ex(target[i], source[i])  // SliceCSGMesh.hpp:25-26
            let r = crate::clipper_utils::difference(&target[i], &source[i]);
            target[i] = r;
        }
        CSGType::Intersection => {
            // target[i] = intersection_ex(target[i], source[i])  // SliceCSGMesh.hpp:28-29
            let r = crate::clipper_utils::intersection(&target[i], &source[i]);
            target[i] = r;
        }
    }
}
