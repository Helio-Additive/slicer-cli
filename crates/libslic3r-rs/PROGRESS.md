...
|---|---|---|---|
| 2026-06-10 | FIX #5 | SCALING_FACTOR 1e-6→1e-5 (10x XY explosion) + layer-0 mid-plane slice_z | Bisect localized a 149x filament regression (3817.65→570,107mm, 113,974→1.54M gcode lines) to checkpoint 54792d7's faithful TriangleMeshSlicer port. Root (workflow-audited + empirically verified): `src/libslic3r.rs:17` had `SCALING_FACTOR = 0.000001` (PrusaSlicer value; comment misquoted reference — BambuStudio libslic3r.h:58 is `0.00001`), so `scaled_f32` produced 1e6-units/mm vertices vs the crate-wide 1e5 convention → all XY 10x too large (100x area), gap-closing radius effectively 0.2mm vs C++ 2mm, f32 ulp 8-16 units → degenerate-line debug_assert (rs:294). FIX: constant→0.00001 (also repairs fill.rs/g_code.rs/support_parameters.rs/bounding_box.rs importers); `scaled_f32` = plain f32 division per C++ `scaled<float>` (Point.hpp:529, no +0.5/floor); layer-0 `slice_z` = mid-plane `min_z+0.5*first_layer_height` (PrintObjectSlice.cpp:36; was top-of-layer). Added `[profile.parity]` (dev minus debug-assertions) — parity slices must use it: golden gcode is from C++ release where `assert()` is noop; the faithful rs:294 assert legitimately fires on Benchy in dev. RESULT: filament **3818.67** (golden 3858.97, 0.99x), 114,210 lines, 240 layers, features match baseline (Outer 735/Inner 717/Gap 203/Sparse 198/Top 5). Confirmed-but-deferred to config-threading track: slice_closing_radius=0.049 + resolution=0.0025 never threaded into slice_mesh (make_expolygons ignores closing_radius/fill_type; needs offset2_ex morphological closing per TriangleMeshSlicer.cpp:1796-1823); latent vase-mode keep_largest_contour_only divergence. |
